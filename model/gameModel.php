...
{
    // Maak hier de code om een game toe te voegen
...
{
    // Maak hier de code om een game te bewerken
    $conn = openDatabaseConnection();
    ...
    // UPDATE employees set name = '$data[name]', age = $data[age]
    $result = $stmt->execute();
    $conn = null;
    return $result;
}
...
{
    // Maak hier de code om een game te verwijderen
    $conn = openDatabaseConnection();
...
{
    $conn = openDatabaseConnection();
    $stmt = $conn->prepare("SELECT games.* , gamesplanned.* FROM games INNER JOIN gamesplanned ON games.id = gamesplanned.gameId");
    $result = $stmt->execute();
    $result = $stmt->fetchAll();
    $dbConn = null;
    return $result;
